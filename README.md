...
기존 DDD Modeling, DB Migration, API Implementation, Frontend Implementation, API Connect, Test Implementation runner는 상태 기록과 반복 실행을 위한 호환층으로 남깁니다.
자동 구현 능력이 부족한 작업은 `needs_human`으로 멈추고, 어떤 playbook 또는 runner capability가 부족한지 보고합니다.
...
agents/        Agent abstraction, Markdown specs/playbooks, legacy runner adapters
