...
UI/UX 방향성을 먼저 잡아야 하는 작업은 이슈 생성 전에 `uiux`를 실행한다. 화면 증거 이미지를 Git에 올릴 때는 `publish-ui-evidence`로 `stage` 병합까지 완료한다.
...
harness uiux \
  ...
  --target-user "신규 사용자" \
  --route / \
  --note "첫 화면에서 다음 행동을 명확히 한다."

harness create-issue \
  --type feFeature \
  --title "[FE] 메인 화면 온보딩 개선" \
  --body-file artifacts/<task_id>/uiux-designer/planning-handoff.md

harness publish-ui-evidence \
  --image artifacts/<task_id>/uiux-designer/screenshots/desktop-home.png \
  --issue 8
```
...
GitHub issue를 생성하고 하네스 DB task로 동기화한다.

```bash
harness create-issue --type feFeature --title "[FE] 메인 화면 온보딩 개선" --body-file ./notes/uiux-issue.md
```

동작:

- GitHub issue를 생성한다.
- 로컬 SQLite `tasks` 테이블에는 새 task를 `Backlog`로 저장한다.
- `GITHUB_PROJECT_NUMBER`가 설정되어 있으면 GitHub Project item도 `Backlog`로 이동한다.
- 새 issue가 Project에 없으면 Project에 추가한 뒤 `Backlog`로 이동한다.

### `harness publish-ui-evidence`

UI/UX 이슈에 첨부할 화면 증거 이미지를 target repo에 커밋하고 `stage` 브랜치에 병합한다.

```bash
harness publish-ui-evidence --image ./artifacts/uiux/screenshots/home-desktop.png --issue 8
```

동작:

- 이미지를 target repo의 `docs/qa-screenshots/<slug>/` 아래로 복사한다.
- `qa-assets/<slug>` 브랜치에서 이미지 커밋을 만든다.
- `DEVELOPMENT_BASE_BRANCH` 기본값인 `stage` 브랜치로 `--no-ff` 병합한다.
- 기본적으로 증거 브랜치와 `stage`를 원격에 push한다.

주의:

- target repo working tree가 깨끗해야 한다.
- 로컬 검증만 할 때는 `--no-push`를 붙인다.
...
- 상태가 `Dev Ready`인지 확인한다.
- Dev Agent를 실행한다.
- 작업 타입에 맞는 runner를 선택한다.
- 가능한 경우 코드를 수정하고 커밋한다.
- 실행이 끝나면 상태를 `Dev Review`로 맞춘다.
- 개발 artifact를 생성한다.
...
- 사람이 Plan을 보고 `harness approve --stage plan`으로 승인했을 때
...
- 상태가 `QA Review`인지 확인한다.
- QA Agent를 다시 실행한다.
- 상태는 `QA Review`로 유지한다.
...
```json
{
  ...
  "state": "Dev Review",
  "next": "harness approve --stage dev",
  "latest_run": {
    "agent": "dev",
    "status": "success",
...
각 단계의 Human Approval Gate를 기록한다.

```bash
harness approve --issue 8 --stage plan --approved-by rsy --notes "설계를 확인했다."
harness approve --issue 8 --stage dev --approved-by rsy --notes "구현 결과를 확인했다."
harness approve --issue 8 --stage qa --approved-by rsy --notes "QA 결과를 확인했다."
harness approve --issue 8 --stage deploy --approved-by rsy --notes "배포 가능 상태로 승인한다."
```

동작:

- `Plan Review`에서 plan 승인 시 `Dev Ready`로 전환한다.
- `Dev Review`에서 dev 승인 시 `QA Ready`로 전환한다.
- `QA Review`에서 qa 승인 시 `Ready To Deploy`로 전환한다.
- `Ready To Deploy`에서 deploy 승인 시 `Done`으로 전환한다.
- 승인자와 승인 메모를 audit log에 남긴다.

주의:

- 일반적으로 `--issue`를 사용한다.
- `--task-id` 기반 승인은 과거 Human QA 승인 호환용으로 남아 있다.
...
- `Plan Review`: Plan Agent가 완료되어 사람이 설계를 검토하는 상태
- `Dev Ready`: Plan 승인이 끝나 Dev Agent를 실행할 수 있는 상태
- `Dev Review`: Dev Agent가 완료되어 사람이 구현 결과를 검토하는 상태
- `QA Ready`: Dev 승인이 끝나 QA Agent를 실행할 수 있는 상태
- `QA Review`: QA Agent가 완료되어 사람이 QA 결과를 검토하는 상태
- `Ready To Deploy`: QA 승인이 끝나 최종 배포 승인만 남은 상태
- `Done`: 사람이 배포 가능 상태까지 최종 승인한 상태
- `Cancelled`: 작업 중지
...
### `QA는 QA Ready에서만 실행할 수 있습니다`

QA는 Dev Review 승인 이후에만 실행된다.
...
### `re-QA는 QA Review에서만 실행할 수 있습니다`
